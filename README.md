...
- 기본 정보 API를 활용하여 전문 병원의 위치를 보여줍니다. 사용자의 위치 정보와 진료과를 파라미터 값으로 XML 형태의 병원 기본 정보 API를 호출하였습니다. 해당 데이터를 JSOUP 라이브러리로 파싱 하여 각 병원의 이름, 주소, 전화번호, 그리고 요양코드를 추출하였습니다. 추출한 정보들은 카카오 지도에서 병원 마커를 클릭 하면 확인할 수 있도록 구현하였습니다.
...
- 탐색한 시간대별로 해당 시간대에 운영 중인 전문 병원들을 직관적으로 확인할 수 있습니다. 병원 상세 정보 API에서 추출한 운영 시간 데이터를 활용하여, 각 병원마다 진료 중인 경우에는 초록색 마커로 표시가 되게 하였으며, 진료를 할 수 없는 시간대인 경우에는 빨간색 마커로 표시가 되게 구현하였습니다.
