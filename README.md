...
- **Kakao 지도를 활용하여 병원의 위치를 확인하고,**
- **ChatGPT 기반의 AI 자가진단으로 더욱 효율적인 병원 탐색을 구현하였습니다.**

**[ 맡은 역할 ]**
- 프로젝트 기획, 병원 기본정보 API, 병원 상세정보 API, 시간대별 마커 전환
...
- 사용자는 신체 부위를 선택하고 증상을 선택 시, 진료과가 정해집니다.
- 그리고 사용자의 현재 위치 정보를 받아 병원 기본 정보 API를 호출합니다.
- 병원의 기본 정보 API 중 요양코드를 파라미터 값으로 전달하여 병원 상세 정보 API를 한번 더 호출합니다.
- 병원 상세 정보 API에는 진료 시간에 따른 병원 마킹 기능에 필요한 진료 시간 데이터가 저장되어 있습니다.
- 2개의 API는 프론트단에 맞는 데이터 형식으로 파싱되어 전송되고 지도에 출력됩니다.
- 사용자는 직관적으로 자신의 현재 위치 주변의 병원들을 확인할 수 있습니다.
- 마킹 표시로 현재 진료가 가능한 병원 등의 주요 정보를 열람할 수 있습니다.

<img src="https://github.com/user-attachments/assets/8aee4601-32c7-4598-bc6a-b1722d22ec58"  width="70%" />

- OpenAI API를 활용한 AI 챗봇 기능입니다. AI 챗봇은 사용자의 자가 진단을 도와줍니다.
- 사용자는 증상 버튼을 누르거나 채팅으로 작성 시 해당 정보를 파라미터 값으로 전달하여 OpenAI API를 호출할 수 있습니다.
...
- 각 부위를 선택 시, 증상 선택으로 전환됩니다.
- 해당 부위에 대한 증상 목록을 확인하고 선택할 수 있습니다.
- 증상 선택 후 완료 시, 자동으로 지도 화면으로 이동합니다.
- 현재 사용자의 위치를 파란색 마커로 확인할 수 있습니다.
- 사용자의 위치를 기준으로, 사용자의 증상을 진단할 수 있는 병원들의 위치 및 정보를 추가로 확인할 수 있습니다.
...
- OpenAI API를 활용하여 자가 진단을 도와주는 챗봇을 구현하였습니다. 정확한 진료과만 선별될 수 있도록 프롬프트를 지정하였습니다.
- 사용자는 채팅 혹은 버튼을 통해 자신의 증상 정보를 파라미터 값으로 전달합니다.
- 값에 대해 ChatGPT가 진단한 정보는 JSON 형태로 전송되어 필요한 정보만을 추출한 후 String 형태로 전송됩니다.
...
- 해당 데이터를 JSOUP 라이브러리로 파싱 하여 각 병원의 이름, 주소, 전화번호, 그리고 요양코드를 추출하였습니다.
- 추출한 정보들은 카카오 지도에서 병원 마커를 클릭 하면 확인할 수 있도록 구현하였습니다.
- 상세 정보 API를 활용하여 해당 병원의 상세 정보를 보여줍니다.
- 이전에 추출한 요양코드를 파라미터 값으로 하여 JSON 형태의 병원 상세 정보 API를 호출하였습니다.
- 해당 데이터도 JSOUP 라이브러리로 파싱 하여 각 병원의 주요 정보와 진료 시간을 추출하였습니다.
...
- 병원 상세 정보 API에서 추출한 운영 시간 데이터를 활용하였습니다.
- 각 병원마다 진료 중인 경우에는 초록색 마커로 표시가 되게 하였습니다.
...
- 우측 상단 건강 퀴즈 버튼을 통해 건강과 관련된 퀴즈를 풀 수 있습니다.
- 랜덤한 문제들이 나오고 퀴즈를 푼 후에 답을 확인할 수 있습니다.
